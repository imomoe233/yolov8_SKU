...
class ToolHelper():
    ...
    def parse_xml(file_path):
        '''
        Parses an XML file and extracts bounding box information along with other attributes.
        Handles cases where certain expected elements might be missing.

        Args:
            file_path (str): Path to the XML file.

        Returns:
            list: A list of dictionaries, each containing bounding box information and metadata.
        '''
        try:
            ...
            for obj in objects:
                data = {}
                name = obj.find('name')
                data['name'] = name.text if name is not None else "Unknown"

                robndbox = obj.find('robndbox')
                if robndbox is not None:
                    data['cx'] = float(robndbox.find('cx').text) if robndbox.find('cx') is not None else 0
                    data['cy'] = float(robndbox.find('cy').text) if robndbox.find('cy') is not None else 0
                    data['w'] = float(robndbox.find('w').text) if robndbox.find('w') is not None else 0
                    data['h'] = float(robndbox.find('h').text) if robndbox.find('h') is not None else 0
                    data['angle'] = float(robndbox.find('angle').text) if robndbox.find('angle') is not None else 0
                else:
                    data['cx'] = data['cy'] = data['w'] = data['h'] = data['angle'] = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img_path', type=str, default='D:\code\datasets/val_test_dataaug/images')
    parser.add_argument('--source_xml_path', type=str, default='D:\code\datasets/val_test_dataaug/labels')
    parser.add_argument('--save_img_path', type=str, default='D:\code\datasets/val_test_dataaug/images2')
    parser.add_argument('--save_xml_path', type=str, default='D:\code\datasets/val_test_dataaug/labels2')
